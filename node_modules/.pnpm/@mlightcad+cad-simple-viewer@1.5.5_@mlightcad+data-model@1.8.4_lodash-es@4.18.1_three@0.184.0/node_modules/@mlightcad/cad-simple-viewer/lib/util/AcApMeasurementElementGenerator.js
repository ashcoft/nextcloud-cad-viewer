...
export function measurementColor(db) {
    return AcDbSysVarManager.instance().getVar(AcDbSystemVariables.MEASUREMENTCOLOR, db);
}
/** Converts an AcCmColor to a CSS rgba() string. */
export function colorToCssAlpha(c, alpha) {
    return "rgba(".concat(c.red, ", ").concat(c.green, ", ").concat(c.blue, ", ").concat(alpha, ")");
}
/** Returns the CSS color string for a measurement color, with fallback. */
export function cssColor(c) {
    var _a;
    return (_a = c.cssColor) !== null && _a !== void 0 ? _a : "rgb(".concat(c.red, ", ").concat(c.green, ", ").concat(c.blue, ")");
}
/** Creates a small dot element for persistent overlays (CSS2DRenderer). */
export function makeDot(c) {
    var el = document.createElement('div');
    el.style.cssText =
        'width:12px;height:12px;border-radius:50%;' +
            "background:".concat(cssColor(c), ";border:2px solid var(--ml-ui-border, #fff);box-sizing:border-box;") +
            'pointer-events:none;transform:translate(-50%,-50%);';
    return el;
}
/** Creates a badge element for persistent overlays (CSS2DRenderer). */
export function makeBadge(c, text) {
    if (text === void 0) { text = ''; }
    var el = document.createElement('div');
    el.textContent = text;
    el.style.cssText =
        "background:var(--ml-ui-bg, rgba(255,255,255,0.95));color:".concat(cssColor(c), ";") +
 ...
 */
export function makeLiveBadge(c) {
    var el = document.createElement('div');
    el.style.cssText =
        "position:fixed;background:var(--ml-ui-bg, rgba(255,255,255,0.95));color:".concat(cssColor(c), ";") +
            'font-size:13px;font-family:sans-serif;font-weight:500;' +
            'padding:3px 14px;border-radius:20px;pointer-events:none;' +
            'transform:translate(-50%,-50%);white-space:nowrap;' +
 ...
 * Caller is responsible for calling `.remove()` when done.
 */
export function makeSnapIndicator(c) {
    var el = document.createElement('div');
    el.style.cssText =
        "position:fixed;width:10px;height:10px;border:2px solid ".concat(cssColor(c), ";") +
 ...
 * Caller is responsible for calling `.remove()` when done.
 */
export function makeOverlayCanvas(container) {
    var el = document.createElement('canvas');
    el.style.cssText = 'position:absolute;pointer-events:none;z-index:1;';
    container.appendChild(el);
    return el;
}
//# sourceMappingURL=AcApMeasurementElementGenerator.js.map