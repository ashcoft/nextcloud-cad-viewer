      ...
      for (let h = 0; h < e - 1; h++)
        try {
          ...
          if (c > 0) {
            ...
            if (l.length === c) {
              const { name: u, bytecode: b } = I(l);
              b.length > 0 && (o[a] = b, u && (r[u] = a));
            }
          }
        } catch {
...
class M {
  ...
  constructor(t) {
    if (t.start && t.end && t.bulge !== void 0) {
      ...
      const o = this.start.clone().add(e.clone().divide(2)), r = new d(-e.y, e.x);
      ...
      this.endAngle = this.startAngle + (this.isClockwise ? -e : e), this.start = this.center.clone().add(
        ...
        new d(this.radius * Math.cos(this.endAngle), this.radius * Math.sin(this.endAngle))
      );
    } else
  ...
  tessellate(t = Math.PI / 18) {
    ...
    for (let i = 1; i < n; i++) {
      ...
      e.push(
        this.center.clone().add(new d(this.radius * Math.cos(r), this.radius * Math.sin(r)))
      );
    }
    return e.push(
...
class A {
  ...
  normalizeToOrigin(t = !1) {
    const e = this.bbox;
    return this.offset(new d(-e.minX, -e.minY), t);
  }
  /**
  ...
  toSVG(t = {}) {
    ...
    let i, o;
    if (n) {
      const r = this.bbox, h = 0.2, a = r.maxX - r.minX, c = r.maxY - r.minY, l = a === 0 ? c : a, u = c === 0 ? a : c, b = r.minX - l * h, P = r.maxX + l * h, m = r.minY - u * h, C = r.maxY + u * h;
...
class $ {
  ...
  parseAndScale(t, e) {
    ...
    if (this.shapeCache.has(t))
      ...
      if (n[t]) {
        const i = n[t];
        s = this.parseShape(i), this.shapeData.set(t, s), this.shapeCache.set(t, s);
      }
    }
    if (s) {
      if (e.factor !== void 0)
  ...
  handleOctantArc(t, e, s) {
    ...
    let h = o & 7;
    const a = o < 0, c = Math.PI / 4 * r, l = s.currentPoint.clone().subtract(new d(Math.cos(c) * i, Math.sin(c) * i)), u = M.fromOctant(l, i, r, h, a);
    if (s.isPenDown) {
      const P = u.tessellate();
      s.currentPolyline.pop(), s.currentPolyline.push(...P.slice());
    }
    return s.currentPoint = (b = u.tessellate().pop()) == null ? void 0 : b.clone(), n;
  }
  handleFractionalArc(t, e, s) {
    ...
    const k = s.currentPoint.clone().subtract(new d(a * Math.cos(D), a * Math.sin(D)));
    if (s.currentPoint = k.clone().add(new d(a * Math.cos(x), a * Math.sin(x))), s.isPenDown) {
      let g = D;
      const S = [];
      if (S.push(
        k.clone().add(new d(a * Math.cos(g), a * Math.sin(g)))
      ), m > 0)
        for (; g + m < x; )
          g += m, S.push(
            k.clone().add(new d(a * Math.cos(g), a * Math.sin(g)))
      ...
      S.push(k.clone().add(new d(a * Math.cos(x), a * Math.sin(x)))), s.currentPolyline.push(...S);
    }
    return n;
  }
  ...
  getScaledSubshapeAtInsertPoint(t, e, s, n, i = !1) {
    ...
    if (i) {
      ...
      if (!a)
        return;
      o = this.parseShape(a, { initialPenDown: !0 });
    } else if (o = this.shapeCache.get(t), !o) {
      const a = this.fontData.content.data[t];
      if (!a)
        return;
      o = this.parseShape(a), this.shapeData.set(t, o), this.shapeCache.set(t, o);
    }
