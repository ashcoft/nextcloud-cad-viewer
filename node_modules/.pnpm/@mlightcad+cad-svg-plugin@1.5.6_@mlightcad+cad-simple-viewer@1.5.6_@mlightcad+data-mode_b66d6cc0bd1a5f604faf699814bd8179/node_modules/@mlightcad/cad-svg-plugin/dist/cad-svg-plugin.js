import { AcGeBox2d as C, AcGePoint3d as j, AcGiContext as G, AcGiMTextFlowDirection as E, AcGiMTextAttachmentPoint as p, AcCmColor as U, AcCmTransparency as q, AcGiLineWeight as K, AcDbRenderingCache as Z } from "@mlightcad/data-model";
import { MTextContext as I, MTextParser as J, MTextParagraphAlignment as S, TokenType as v } from "@mlightcad/mtext-parser";
import { FontManager as Q, ShxParserFont as tt } from "@mlightcad/mtext-renderer";
import { resolveExportDownloadName as it, AcApSettingManager as et, AcEdCommand as st, AcEdCommandStack as nt } from "@mlightcad/cad-simple-viewer";
import { SVG_PLUGIN_NAME as oi, SVG_PLUGIN_TRIGGERS as ai } from "./cad-svg-plugin-register.js";
class X {
  /**
...
class y {
  constructor() {
    this._objectId = "", this._ownerId = "", this._layerName = "", this._visible = !0, this._userData = {}, this._box = new C(), this._localSvg = "";
  }
  /**
...
class l {
  ...
  static resolveRgb(t, i, e) {
    return e === "fill" && t.color.isForeground && this.isSolidBackgroundHatch(t) ? i.backgroundColor : G.fromBackgroundColor(
      i.backgroundColor
    ).resolveSubEntityTraitsRgb(t);
  }
  ...
  static formatAttributes(t) {
    return Object.entries(t).map(([i, e]) => `${i}="${rt(e)}"`).join(" ");
  }
  static tag(t, i, e) {
...
class ct extends y {
  constructor(t, i, e) {
    super();
    const n = t.getPoints(ht).reduce((a, h, c) => (a += c === 0 ? "M" : "L", a += `${h.x},${h.y}`, a), "");
    if (n) {
      const a = {
        d: n,
...
class ut extends y {
  constructor(t, i, e) {
    super();
    const n = t.getPoints(lt).reduce((a, h, c) => (a += c === 0 ? "M" : "L", a += `${h.x},${h.y}`, a), "");
    if (n) {
      const a = {
        d: n,
        ...l.strokeAttributes(i, e)
...
const V = /\.(shx|ttf|otf|woff2?)$/i, xt = {
...
function W(o, t) {
  ...
  const e = B(o), s = N[e] ?? xt[e] ?? N[o.toLowerCase()];
  return s || (V.test(o) ? i : o.includes(",") ? o : `"${o}", ${i}`);
}
function bt(o) {
  if (!(o != null && o.trim()))
    return 1;
  const t = B(o), i = z[t] ?? yt[t] ?? z[o.toLowerCase()];
  if (i == null)
    return 1;
  const e = Number(i);
...
const St = 0.25, vt = 1.666666, Tt = 0.3, wt = 0.6, _t = 1, Lt = 4, At = 0.7;
function Mt(o) {
  if (!o || o.length !== 1)
    return !1;
...
function Pt(o, t) {
  const i = typeof t.lineSpaceFactor == "number" ? t.lineSpaceFactor : St;
  return o * (1 + i * vt);
}
function Rt(o) {
...
function It(o, t) {
  const i = Ft(t), e = (i.minX + i.maxX) / 2, s = (i.minY + i.maxY) / 2;
  switch (o) {
    case p.TopCenter:
      return { x: -e, y: -i.maxY };
    case p.TopRight:
...
function Yt(o, t, i, e) {
  const s = new C(), { text: n, height: r, position: a } = o;
  if (!n || r <= 0)
    return { localSvg: "", box: s };
  const h = r, c = Pt(h, o), m = Rt(o), u = Et(o.drawingDirection), g = W(
  ...
  x.capHeight = { value: h, isRelative: !1 }, x.widthFactor = { value: f, isRelative: !1 }, x.paragraph.align = Ht(o), typeof t.obliqueAngle == "number" && (x.oblique = t.obliqueAngle), x.fontFace.family = $(
  ...
  for (const k of _.parse())
    d.consume(k);
  d.finish();
  const b = d.bounds, L = It(o.attachmentPoint, b), { x: A, y: M } = a, T = Ct(o) * 180 / Math.PI, w = [`translate(${A},${M})`];
  ...
  return Bt(s, b, L, A, M), { localSvg: R, box: s };
}
function Bt(o, t, i, e, s) {
...
class Wt {
  ...
  get tspanMarkup() {
    ...
    for (const e of this.lines) {
      ...
      for (const r of e.spans) {
        ...
        this.isVertical() ? (a.x = String(r.x), a.y = String(r.y)) : h ? (s = r.x, a.x = String(r.x), a.dy = i ? "0" : String(this.lineAdvance), i = !1, n = r.x + this.measureText(r.text, r.fontSize, r.tokenCtx)) : n != null && Math.abs(r.x - n) > 1e-6 ? (a.x = String(r.x), n = r.x + this.measureText(r.text, r.fontSize, r.tokenCtx)) : n = (n ?? r.x) + this.measureText(r.text, r.fontSize, r.tokenCtx), t += l.tag("tspan", a, $t(r.text));
      }
    }
    return t;
  }
  ...
  emitStack(t, i) {
    ...
    if (this.setPosition(r.x + g, r.y - h * 0.35), this.isLineStart = !0, this.emitText(e, i), this.setPosition(r.x + f, r.y + h * 0.2), this.isLineStart = !0, this.emitText(s, i), n === "/" || n === "#") {
      const d = r.y - h * 0.05 + this.baseHeight * Tt;
  ...
  advanceToTab(t) {
    const i = this.baseHeight * Lt, e = t.paragraph.tabs;
    if (e.length === 0) {
  ...
  measureCharWidth(t, i, e) {
    const s = this.resolveWidthFactor(e), n = this.resolveTracking(e), r = Mt(t) ? _t : wt;
    return i * r * s * n;
  }
  measureText(t, i, e) {
    let s = 0;
...
class zt extends y {
  constructor(t, i, e, s) {
    super();
    const n = i.displaySize > 0 ? i.displaySize / 2 : Nt, r = {
      cx: String(t.x),
      cy: String(t.y),
      r: String(n),
...
function Ot(o, t, i, e) {
  const s = new C(), n = { localSvg: "", box: s }, r = o.size, a = o.position;
  if (!a || !(r > 0))
    return n;
  const h = Vt(o, t);
  if (!(h != null && h.length))
    return n;
  const c = o.widthFactor ?? t.widthFactor ?? 1, m = (t.obliqueAngle ?? 0) * Math.PI / 180, u = m !== 0 ? Math.tan(m) : 0, g = Dt(h, c, u);
  if (g.length === 0)
    return n;
  const f = Gt(g), x = Ut(o.rotation, o.directionVector), _ = Math.cos(x), d = Math.sin(x), b = l.strokeAttributes(i, e), L = [];
  for (const A of g) {
    let M = "";
    for (let T = 0; T < A.length; T++) {
      const w = A[T].x + f.x, H = A[T].y + f.y, P = w * _ - H * d + a.x, R = w * d + H * _ + a.y;
...
function Vt(o, t) {
  var m;
  const i = $(t.font), e = o.size, s = (m = o.name) == null ? void 0 : m.trim(), n = o.shapeNumber, r = Q.instance, a = s && r.getShapeByName(s, i, e) || (n != null && n !== 0 ? r.getShapeByCode(n, i, e) : void 0);
  if (a)
    return kt(a);
  const h = r.getFontByName(i, !1);
  if ((h == null ? void 0 : h.type) !== "shx")
    return;
  const c = new tt(h.data);
  try {
...
class D {
  constructor() {
    this._ltscale = 1, this._celtscale = 1, this._currentBackgroundColor = 0, this._foregroundColor = 0, this._showLineWeight = !1, this._entities = [], this._bbox = new C(), this._fontMapping = {}, this._pendingImages = [], this._subEntityTraits = {
      color: new U(),
      lineType: {
        type: "ByLayer",
        name: "Continuous",
   ...
   */
  get context() {
    return G.fromBackgroundColor(this._currentBackgroundColor);
  }
  /**
   * @inheritdoc
   */
   ...
   */
  group(t) {
    return this.removeEntities(t), this.pushEntity(new ft(t));
  }
  /**
   * @inheritdoc
   */
  point(t, i) {
    return this.pushEntity(
      new zt(t, i, this._subEntityTraits, this.styleContext)
    );
  }
  /**
   * @inheritdoc
  ...
  circularArc(t) {
    return this.pushEntity(
      new ct(t, this._subEntityTraits, this.styleContext)
    );
  }
  /**
   * @inheritdoc
  ...
  lines(t) {
    return this.pushEntity(
      new pt(t, this._subEntityTraits, this.styleContext)
    );
  }
  /**
   * @inheritdoc
  ...
  area(t) {
    return this.pushEntity(
      new at(t, this._subEntityTraits, this.styleContext)
    );
  }
  /**
   * @inheritdoc
...
class Kt {
  ...
  async convert(t) {
    ...
    for (const r of i)
      r.worldDraw(e);
    const s = await e.exportAsync(), n = it(
  ...
  configureRenderer(t, i) {
    ...
    t.ltscale = e.ltscale, t.celtscale = e.celtscale, t.showLineWeight = !!e.lwdisplay, t.setFontMapping(et.instance.fontMapping);
...
class Jt {
   ...
   */
  onLoad(t, i) {
    const e = nt.SYSTEMT_COMMAND_GROUP_NAME;
    i.addCommand(e, "csvg", "csvg", new Zt()), this.registeredCommands.push({ group: e, name: "csvg" });
  }
  /**
...
export {
  ...
  Kt as AcApSvgConvertor,
  D as AcSvgRenderer,
  oi as SVG_PLUGIN_NAME,
  ai as SVG_PLUGIN_TRIGGERS,
  si as createSvgPlugin
};
